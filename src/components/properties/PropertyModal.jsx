...
export default function PropertyModal({ property, onClose }) {
  ...
  return (
    <AnimatePresence>
      {property && (
        <motion.div
          className="fixed inset-0 z-[9999] flex items-center justify-center bg-slate-950/45 px-4 backdrop-blur-[2px]"
          onClick={onClose}
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
        >
          <motion.div
            className="iphone-scrollbar relative max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-md border border-white/10 bg-slate-950 text-white shadow-md"
          ...
          >
            <div className="sticky top-0 z-30 h-80 bg-slate-950">
                <button
                    onClick={onClose}
                    className="absolute right-4 top-4 rounded-full bg-slate-950/80 p-3 text-white backdrop-blur transition hover:bg-slate-800"
                    >
                    <X className="h-5 w-5" />
                </button>
            ...
            <div className="relative z-10 bg-slate-950 p-6">
              ...
              <div className="mt-6 rounded-3xl border border-white/10 bg-slate-900/70 p-4">
                ...
                <div className="mt-4 grid grid-cols-2 gap-3">
                  <button className="rounded-2xl bg-blue-600 px-4 py-3 font-semibold text-white hover:bg-blue-500">
                    <Phone className="mr-2 inline h-4 w-4" />
                    Call
                  </button>

                  <button className="rounded-2xl border border-white/20 bg-white/5 px-4 py-3 font-semibold text-white hover:bg-white/10">
